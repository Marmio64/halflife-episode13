...
/datum/pet_command/proc/respond_to_callout(mob/living/clicker, datum/callout_option/callout, atom/target)
	SIGNAL_HANDLER
	...
	if (!valid_callout_target(clicker, callout, target))
		var/found_new_target = FALSE
		for (var/atom/new_target in range(2, target))
			if (valid_callout_target(clicker, callout, new_target))
				target = new_target
	...
	if (try_activate_command(clicker))
		look_for_target(parent, target)

/// Does this callout with this target trigger this command?
/datum/pet_command/proc/valid_callout_target(mob/living/clicker, datum/callout_option/callout, atom/target)
	return TRUE
