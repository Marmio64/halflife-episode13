...
/datum/controller/subsystem/ticker/proc/declare_completion(was_forced = END_ROUND_AS_NORMAL)
	...
	if(SSmapping.current_map.minetype == "combat_deployment")
		to_chat(world, span_infoplain(span_big(span_bold("<BR><BR><BR>This battle has concluded."))))
		to_chat(world, span_infoplain(span_slightly_larger(span_bold("<BR>COMBINE DEATHS: [tdm_combine_deaths]"))))
		to_chat(world, span_infoplain(span_slightly_larger(span_bold("REBEL DEATHS: [tdm_rebel_deaths]"))))
		to_chat(world, span_infoplain(span_slightly_larger(span_bold("TOTAL DEATHS: [SSticker.tdm_total_deaths]"))))
	else
