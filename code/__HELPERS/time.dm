...
/proc/station_time(display_only = FALSE, wtime=world.time)
	return ((((wtime - SSticker.round_start_time) * SSticker.station_time_rate_multiplier) + SSticker.gametime_offset) % MIDNIGHT_ROLLOVER) - (display_only? GLOB.timezoneOffset : 0) //hl13 edit
...
/proc/station_time_debug(force_set)
	if(isnum(force_set))
		...
		return
	SSticker.gametime_offset = rand(0, MIDNIGHT_ROLLOVER) //hours in day * minutes in hour * seconds in minute * deciseconds in second //hl13 edit
	if(prob(50))
