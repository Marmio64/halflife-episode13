...
/obj/item/melee/baton/Initialize(mapload)
	...
	if(stamina_damage != 0)
		offensive_notes = "It takes [span_warning("[CEILING(125 / stamina_damage, 1)] stunning hit\s")] to stun an enemy." //hl13 edit, more stam damage required
...
/obj/item/melee/baton/proc/finalize_baton_attack(mob/living/target, mob/living/user, modifiers, in_attack_chain = TRUE)
	...
	if(baton_effect(target, user, modifiers) && user)
		...
		if(ishuman(target))
			...
			if(M && (M.assigned_role?.title == JOB_ASSISTANT || M.assigned_role?.title == "Prisoner" || M.assigned_role?.title == JOB_PRISONER || isvortigaunt(H)))
				var/quota_to_give = 1
				if(user.combat_mode)
					quota_to_give = 5
				if(H.stat == DEAD)
