/datum/antagonist/brother
	name = "\improper Rebel Cell Member"
	antagpanel_category = "Rebel Cell Member"
	job_rank = ROLE_BROTHER
	var/special_role = ROLE_BROTHER
	antag_hud_name = "brother"
	hijack_speed = 0.5
	ui_name = "AntagInfoBrother"
	suicide_cry = "FOR THE CELL!!"
...
/datum/antagonist/brother/get_preview_icon()
	...
	var/icon/brother2_icon = render_preview_outfit(/datum/outfit/job/engineer, brother2)
...
/datum/antagonist/brother/greet()
	...
	to_chat(owner.current, "Lambda has sent your small cell of brothers here to spur the resistance movement. Prove yourself and prove your [team.member_name]s by completing your objectives together!")
	owner.announce_objectives()
	give_meeting_area()

/datum/antagonist/brother/proc/finalize_brother()
	owner.current.playsound_local(get_turf(owner.current), 'hl13/sound/ambience/particleghost.ogg', 50, FALSE, pressure_affected = FALSE, use_reverb = FALSE)
	team.update_name()
...
/datum/antagonist/brother/admin_add(datum/mind/new_owner,mob/admin)
	...
	var/choice = tgui_input_list(admin, "Choose the cell member.", "Brother", candidates)
	if(!choice)
		return
...
/datum/team/brother_team
	name = "\improper Rebel Cell Members"
	member_name = "rebel cell member"
...
/datum/team/brother_team/proc/forge_single_objective()
	if(prob(50))
		if(LAZYLEN(active_ais()) && prob(100/GLOB.joined_player_list.len))
			...
			add_objective(new /datum/objective/reduce_sociostability, needs_target = FALSE)
		else
			add_objective(new /datum/objective/assassinate/rebel, needs_target = TRUE)
	else
