...
/obj/item/organ/cyberimp/chest/nutriment/on_life(seconds_per_tick, times_fired)
	...
	if(owner.nutrition <= hunger_threshold)
		...
		owner.adjust_nutrition(3.1 * seconds_per_tick) //hl13 edit, pretty much only enough to sustain no movement
		addtimer(CALLBACK(src, PROC_REF(synth_cool)), 45 SECONDS)
