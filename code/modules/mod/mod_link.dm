...
/datum/mod_link/proc/get_other()
	...
	if(!link_call)
		return
	return link_call.clicker == src ? link_call.receiver : link_call.clicker
...
/datum/mod_link/proc/call_link(datum/mod_link/called, mob/user)
	...
	alert.requester_ref = WEAKREF(src)
...
/datum/mod_link_call
	...
	var/datum/mod_link/clicker
	/// The MODlink that is being called.
	var/datum/mod_link/receiver

/datum/mod_link_call/New(datum/mod_link/clicker, datum/mod_link/receiver)
	clicker.link_call = src
	receiver.link_call = src
	src.clicker = clicker
	src.receiver = receiver
	var/mob/living/clicker_mob = clicker.get_user_callback.Invoke()
	ADD_TRAIT(clicker_mob, TRAIT_IN_CALL, REF(src))
...
/datum/mod_link_call/Destroy()
	var/mob/living/clicker_mob = clicker.get_user_callback.Invoke()
	if(!QDELETED(clicker_mob))
		REMOVE_TRAIT(clicker_mob, TRAIT_IN_CALL, REF(src))
	var/mob/living/receiver_mob = receiver.get_user_callback.Invoke()
	if(!QDELETED(receiver_mob))
		REMOVE_TRAIT(receiver_mob, TRAIT_IN_CALL, REF(src))
	STOP_PROCESSING(SSprocessing, src)
	clear_visuals()
	clicker.link_call = null
...
/datum/mod_link_call/proc/can_continue_call()
	return clicker.frequency == receiver.frequency && clicker.can_call_callback.Invoke() && receiver.can_call_callback.Invoke()

/datum/mod_link_call/proc/make_visuals()
	var/clicker_visual = clicker.make_visual_callback.Invoke()
	var/receiver_visual = receiver.make_visual_callback.Invoke()
	clicker.get_visual_callback.Invoke(receiver_visual)
	receiver.get_visual_callback.Invoke(clicker_visual)

/datum/mod_link_call/proc/clear_visuals()
	clicker.delete_visual_callback.Invoke()
...
/proc/call_link(mob/user, datum/mod_link/calling_link)
	...
	for(var/id in GLOB.mod_link_ids)
		...
		if(!link.can_call_callback.Invoke())
			continue
		clickers["[link.holder] ([id])"] = id
	if(!length(clickers))
		calling_link.holder.balloon_alert(user, "no targets on freq [calling_link.frequency]!")
		return
	var/chosen_link = tgui_input_list(user, "Choose ID to call from [calling_link.frequency] frequency", "MODlink", clickers)
	if(!chosen_link)
		return
	calling_link.call_link(GLOB.mod_link_ids[clickers[chosen_link]], user)
...
/atom/movable/screen/alert/modlink_call/Click(location, control, params)
	...
	if(usr != owner)
		return
	var/datum/mod_link/clicker = requester_ref.resolve()
	var/datum/mod_link/receiver = receiver_ref.resolve()
	if(!clicker || !receiver)
		return
	if(clicker.link_call || receiver.link_call)
		return
	var/list/modifiers = params2list(params)
	if(LAZYACCESS(modifiers, RIGHT_CLICK))
		end_message = "call denied!"
		owner.clear_alert("[REF(clicker)]_modlink")
		return
	end_message = "call accepted"
	new /datum/mod_link_call(clicker, receiver)
	owner.clear_alert("[REF(clicker)]_modlink")
...
/atom/movable/screen/alert/modlink_call/Destroy()
	...
	if(!user || !clicker)
		return ..()
	clicker.holder.balloon_alert(user, end_message)
	return ..()
