...
/mob/living/carbon/proc/breathe(seconds_per_tick, times_fired)
	...
	if(!get_organ_slot(ORGAN_SLOT_BREATHING_TUBE) && !HAS_TRAIT(src, TRAIT_THE_HIDDEN))
		if(health <= HEALTH_THRESHOLD_FULLCRIT || (pulledby?.grab_state >= GRAB_KILL) || (lungs?.organ_flags & ORGAN_FAILING))
			...
			if(prob(get_stat_level(STATKEY_END)/2)) //hl13 edit. If you're in soft crit, there is a small chance you can pull yourself out of it, assuming you're not totally fucked up.
