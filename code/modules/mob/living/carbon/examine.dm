	...
	if(just_sleeping)
		...
		if(brain && isnull(ai_controller))
			...
			if(HAS_TRAIT(brain, TRAIT_GHOSTROLE_ON_REVIVE))
				...
				npc_message = "[t_He] [t_is] totally catatonic. The stresses of life in the post apocalypse must have been too much for [t_him]. Any recovery is unlikely." //hl13 edit
			else if(!client)
