...
/mob/living/proc/ZImpactDamage(turf/impacted_turf, levels)
	...
	var/incoming_damage = (levels * 8) ** 1.75 //HL13 EDIT, falling is more deadly
...
/mob/living/proc/mob_pickup(mob/living/user)
	var/obj/item/clothing/head/mob_holder/holder = new(get_turf(src), src, held_state, head_icon, held_lh, held_rh, worn_slot_flags, worn_flags_inv)
...
/mob/living/proc/process_capture(ransom_price, black_market_price)
	if(ransom_price > 0)
		...
		priority_announce("One of your district's citizens was captured by a hostile actor - we've needed to pay their ransom to bring them back. We've taken from your district's budget to pay for this ransom.", "Worker's Union Asset Protection", has_important_message = TRUE)
