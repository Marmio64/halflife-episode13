...
/obj/machinery/intruder_time_counter/proc/attempt_pick_leaders()
	...
	if(ishuman(candidate_client.mob))
		...
		if(!has_role(H) && new_team_leaders > team_leaders && H.deployment_faction != REBEL_DEPLOYMENT_FACTION)
			...
			to_chat(H, span_notice("Although you and the other squad leaders only have as much authority as everyone else gives you, you automatically go into caution on death, you are capable of supplying your teammates with rations and batteries, and you get a little bit of armor for your head."))
	...
	if(ishuman(candidate_client.mob))
		...
		if(!has_role(H) && !revolver_bullsquid && H.deployment_faction != REBEL_DEPLOYMENT_FACTION)
			...
			if(GLOB.crab_character == "solid")
				H.equipOutfit(/datum/outfit/deployment_loadout/intruder/boss/bullsquid)
			if(GLOB.crab_character == "raiden")
				H.equipOutfit(/datum/outfit/deployment_loadout/intruder/boss/solidus)
			...
			for(var/client/X in GLOB.deployment_combine_players)
				...
				if(GLOB.crab_character == "raiden")
					SEND_SOUND(H_player, 'hl13/sound/voice/solid/solidusappear.ogg')
				if(GLOB.crab_character == "old")
				...
				if(HAS_TRAIT(H_player, TRAIT_THE_INTRUDER))
					...
					if(GLOB.crab_character == "raiden")
						H_player.cmode_music = 'hl13/sound/music/combat/fatherandson.ogg'
						to_chat(H_player, span_notice("Watch out for his charge attack! Melee attacks are more effective against this foe!"))
					if(GLOB.crab_character == "old")
						H_player.cmode_music = 'hl13/sound/music/combat/themeoflove.ogg' //technically this is a ssbb song but all of the songs from the final fight of mgs4 are just borrowed from other games
...
/obj/machinery/intruder_time_counter/process()
	...
	if(GLOB.packages_delivered >= 6)
		GLOB.packages_delivered = 0
		GLOB.bonus_guard_preparedness += 2
	if(GLOB.deployment_flag_grace_period < 1 SECONDS)
		...
		if(combine_players <= SSticker.tdm_combine_deaths && SSticker.IsRoundInProgress())
			...
			to_chat(world, span_infoplain(span_slightly_larger(span_bold("Enough conscripts were killed, that the Intruder wins."))))
	...
	if(GLOB.caution_phases > cautions)
		...
		if(GLOB.complete_objectives_total > objectives)
			...
			GLOB.squad_death = FALSE
			for(var/X in GLOB.deployment_combine_players)
				var/mob/living/carbon/human/H = X
				to_chat(H, "<span class='userdanger'>Communications have ceased with one of our Squad Leaders. Information has been sent to other Squad Leaders, sending reinforcements to investigate.</span>")
				to_chat(H, span_infoplain(span_bold("STATUS: CAUTION")))
		else
			for(var/X in GLOB.deployment_combine_players)
				var/mob/living/carbon/human/H = X
