...
/datum/round_event_control/rebel
	name = "Spawn Armed Rebel"
	typepath = /datum/round_event/ghost_role/rebel
	max_occurrences = 9
	min_players = 8
	weight = 0
	category = EVENT_CATEGORY_ENTITIES
	description = "Spawns a rebel."
	min_wizard_trigger_potency = 6
	max_wizard_trigger_potency = 7

/datum/round_event/ghost_role/rebel
	minimum_required = 1
	role_name = "armed rebel"
	fakeable = FALSE

/datum/round_event/ghost_role/rebel/spawn_role()
	var/mob/chosen_one = SSpolling.poll_ghost_candidates(check_jobban = ROLE_TRAITOR, role_name_text = role_name, amount_to_pick = 1)
	if(isnull(chosen_one))
		return NOT_ENOUGH_PLAYERS
	var/datum/mind/player_mind = new /datum/mind(chosen_one.key)
	...
	player_mind.transfer_to(S)
	player_mind.set_assigned_role(SSjob.get_job_type(/datum/job/prisoner))
	player_mind.special_role = "Armed Rebel"

	S.equipOutfit(/datum/outfit/rebel/medium)
