...
/datum/vapour/proc/ScrubAmount(amount_to_scrub, update_active = TRUE, planetary_multiplier = FALSE)
	...
	var/area/halflife/area_check = get_area(my_turf) //check for indoor/outdoor areas for the planetary multiplier to apply
	//Dissipate faster when outdoors (openspace above us or highest z level)
	if(planetary_multiplier && (!above_turf || istype(above_turf, /turf/open/openspace)))
		//This really should use a blacklist not a fucking whitelist, but whatever
		var/static/list/area_whitelist = list(
			/area/halflife = TRUE,
			/area/halflife/outdoors = TRUE,
			/area/halflife/outdoors/alley = TRUE,
			/area/halflife/outdoors/forest = TRUE,
			/area/halflife/outdoors/plaza = TRUE,
			/area/halflife/outdoors/roofs = TRUE,
			/area/halflife/outdoors/sewage_dump = TRUE,
			/area/halflife/outdoors/tracks = TRUE,
		)
...
/datum/vapour/proc/GetOverlay(list/vapours_list, total_amount)
	...
	SET_PLANE_EXPLICIT(overlay, PLANE_TO_TRUE(VAPOUR_PLANE), my_turf)
	overlay.pixel_x = -8
	overlay.pixel_y = 6
