...
/obj/halflife/fire
	...
	desc = "Ouch!"
	anchored = TRUE
	icon = 'hl13/icons/effects/fire.dmi'
	icon_state = "red_2"
	layer = LOW_OBJ_LAYER
...
/obj/halflife/fire/extinguish()
	qdel(src)
	return

/obj/halflife/fire/proc/on_entered(datum/source, mob/living/M) //Only way to get it to reliable do it when you walk into it.
	SIGNAL_HANDLER
	if(istype(M))
...
/obj/halflife/fire/proc/updateicon()
