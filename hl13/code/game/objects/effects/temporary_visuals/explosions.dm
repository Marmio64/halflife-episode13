...
/particles/dirt_kickup
	icon = 'hl13/icons/effects/96x157.dmi'
	icon_state = "smoke"
	width = 500
	height = 500
	count = 80
...
/particles/falling_debris
	icon = 'hl13/icons/effects/generic_particles.dmi'
	icon_state = "rock"
	width = 750
	height = 750
	count = 75
...
/obj/effect/temp_visual/explosion/Initialize(mapload, radius, color, small = FALSE, large = FALSE, tiny = FALSE)
	...
	if(istype(get_turf(src), /turf/open/halflife/water))
		icon_state = null
		return
	var/image/I
	if(tiny)
		icon = 'icons/effects/64x64.dmi'
		I = image(icon, src, icon_state, 10, 0, -16)
	else
		I = image(icon, src, icon_state, 10, 0, -16)
...
/obj/effect/temp_visual/explosion/proc/generate_particles(radius, small = FALSE, large = FALSE, tiny = FALSE)
	...
	if(istype(turf_type, /turf/open/halflife/water))
		if(tiny)
